...
bool allowauthconnect(int priv = PRIV_ADMIN)
{
...
void authsucceeded(int m, uint id, int priv = PRIV_AUTH)
{
    ...
    if(ci->connectauth)
    {
        if(allowauthconnect(priv) || (ci->_xi.wlauth && !strcmp(ci->_xi.wlauth, ci->authdesc)))
        {
            connected(ci);
            connecting = true;
        }
    ...
    if(om < 0)
    {
        if(ci->authchallenge && checkchallenge(val, ci->authchallenge))
        {
            ...
            if(u)
            {
                ...
                if(ci->connectauth)
                {
                    if(allowauthconnect(u->privilege) || (ci->_xi.wlauth && !strcmp(ci->_xi.wlauth, desc)))
                    {
                        connected(ci);
                        connecting = true;
                    }
